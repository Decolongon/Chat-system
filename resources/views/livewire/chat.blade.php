<div class="flex h-screen" x-init="Echo.private('messages.{{ auth()->id() }}')
    ...
    <div
        class="w-full md:w-1/4 bg-gray-100 border-r border-gray-300 flex-col flex {{ $selectedUser ? 'hidden md:flex' : 'flex' }}">
        ...
        <div class="overflow-y-auto h-[calc(100vh-80px)]">
            @foreach ($this->getUsers as $user)
                <div
                    class="flex items-center hover:bg-gray-200 transition duration-200 {{ $selectedUser == $user->id ? 'bg-blue-100' : '' }}">
                    <button wire:click="selectUser({{ $user->id }})"
                        class="w-full p-4 flex items-center space-x-3 text-left">
        ...
        @if ($selectedUser)
            ...
            <div class="p-4 border-b border-gray-300 bg-white">
                <div class="flex items-center space-x-3">
                    ...
                    <button class="md:hidden mr-2" wire:click="clearSelectedUser" type="button">
            ...
            <!-- Messages Area -->
            <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50" id="messages-container" {{-- wire:igonore --}}>
                @foreach ($this->getMessages as $msg)
                    <livewire:partials.message-item :msg="$msg" :key="$msg->id" />
                @endforeach
            </div>
